...
**The Interface**:
* `reserveInventory(SkuID, UserID, AdmissionToken, ReservationToken)`: Attempt to secure a temporary reservation for one unit of stock.

*Architectural Note: A quick distinction is required here. The `AdmissionToken` proves that the user is allowed to enter the protected core (issued by the Waiting Room). The `ReservationToken` identifies one specific inventory reservation attempt. The reservation token becomes the unique idempotency and correlation key across Redis, SQL, payment, and reconciliation.*
...
if current_stock and current_stock > 0 then
    ...
    redis.call('EXPIRE', res_key, ttl_seconds)
...
Instead of storing all 1,000 units of stock under a single global key, we partition the stock into N distinct buckets distributed across the Redis cluster (e.g., 10 logical shards × 100 units, ideally distributed across Redis cluster slots or nodes).
...
*Shard stealing must be bounded. Otherwise, a sold-out shard can trigger internal retry storms across the Redis cluster.*
...
1. **Phase 1 (Reserve)**: The API validates the `AdmissionToken`, generates a secure, server-issued `ReservationToken`, and calls the Redis Lua script to secure the stock, create the metadata payload, and index it.
...
sequenceDiagram
    ...
    C->>API: 1. POST /checkout (AdmissionToken, ReservationToken)
    API->>R: 2. Lua: Idempotency Check, DECR, HSET, ZADD
    
    alt Stock Reserved In-Memory
        R-->>API: Return Success (ReservationToken Granted)
        API->>DB: 3. Materialize Idempotent Order by ReservationToken
        API-->>C: 4. HTTP 200 (Proceed to Payment)
    else Shard Sold Out
        R-->>API: Return Failure (0 Remaining)
        Note over API: Triggers Bounded Shard Stealing<br/>on adjacent keys
