...
3. **Phase 4 (Commit)**: If the user completes payment before the expiry, the SQL order transitions to COMMITTED, locking in the sale.
4. **Phase 5 (Release & Reconcile)**: A background worker—The Reconciliation Sweeper—queries the Redis Sorted Set using ZRANGEBYSCORE to find tokens that have expired.
