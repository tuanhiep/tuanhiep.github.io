...
* **Admitted Window**: 10,000 valid token holders released into the core checkout path.
...
* **Payment Succeeds After Reservation Expiry**: Validation fails at the capture step; system auto-refunds or voids the transaction.
...
* The release path in the Sweeper must also be atomic. If two sweeper instances scan the same expired token simultaneously, only one must be able to execute the refund Lua script.
