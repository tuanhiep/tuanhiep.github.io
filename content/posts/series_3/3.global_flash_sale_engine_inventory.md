...
sequenceDiagram
    ...
    C->>API: 1. POST /checkout (AdmissionToken, ClientRequestID)
    Note over API: Validate AdmissionToken<br/>Derive ReservationToken
    ...
    W->>DB: 6. Conditional Update:<br/>PENDING -> CANCELLED_BY_SWEEPER
    alt Cancellation Won (RowsAffected = 1)
        W->>R: 7. Lua: Clean Reservation State & INCR Stock Shard
        R-->>W: Stock Refunded Exactly Once
    else Payment Already Committed or Order Not Cancellable
        W->>R: 8. Lua: Clean Expired Metadata Only
        Note over W,R: Do not increment stock
    end
...
* **The SQL-First Cancellation Rule**: The Sweeper must never refund Redis stock merely because a reservation appears expired. It must first win an atomic SQL transition from `PENDING` to `CANCELLED_BY_SWEEPER`, using a conditional update such as `WHERE status = 'PENDING' AND reservation_expires_at < NOW()`. Only if `RowsAffected = 1` may it execute the Redis release script. If the payment path has already moved the order to `COMMITTED`, the Sweeper loses the race and must not increment stock. SQL state transition is the judge; Redis refund is the consequence.
---
