...
**The Single Flight Pattern** ensures that if thousands of concurrent threads ask for the exact same resource (e.g., Product 999), the application server will coalesce them into *one* query to the backend.
...
* **🏛 3) Invariant**: The short-term capacity of the physical database is fixed. Traffic must be forcefully shaped and shed before it crosses the core boundary.
