...
**Production Note:** In production, rate limiting is rarely based on IP alone (which fails spectacularly behind corporate NATs). A flash-sale gateway usually combines user ID, session ID, product ID, device fingerprint, and signed admission tokens to decide who reaches the purchase path.
