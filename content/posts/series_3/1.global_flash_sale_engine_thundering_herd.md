...
* **The Result**: The user perceives zero latency, and the core is protected.
...
* During the first second, 1 million requests arrive. The gateway serves the first 10,000 users.
* For non-admitted users, the gateway either returns a fast `HTTP 429 Too Many Requests` for API clients/bots, or redirects humans to a Virtual Waiting Room. These are different product choices, but architecturally they serve the same purpose: keeping the core below its safe operating limit.
...
**The Single Flight Pattern** ensures that if thousands of concurrent threads ask for the exact same resource (e.g., Product 999), the application server will coalesce them into *one* actual query to the backend.
...
*(Note: In a horizontally scaled fleet, Single Flight usually coalesces per process or per shard; global coalescing requires an additional distributed coordination layer and is often not worth the latency cost).*
...
graph TD
    ...
    subgraph SG2 [2. The Traffic Shaper]
        ...
        U1 -.->|Buy Intent| RL
        RL -->|Rejected| 429([429 / Waiting Room]):::user
    end
...
## ⚗️ 4) The Architect’s Crucible: Scaling Down Before Scaling Up
Junior engineers assume handling high traffic means adding more servers. Principal engineers know that auto-scaling is too slow for a flash sale. You do not scale up to meet a flash sale; you scale the traffic down to fit your existing footprint. 
...
> **🕵️ The Challenger**: If we shape and drop 99% of the traffic at the gateway, aren't we losing potential sales?
...
> **Next up**: We have survived the first second and protected the database. But the users who passed the gateway are now in a holding pattern. How do we ensure fairness and prevent bots from stealing the stock? In **Part 2**, we design the bridge between the Edge and the Inventory: **Admission Control & The Virtual Waiting Room.**
