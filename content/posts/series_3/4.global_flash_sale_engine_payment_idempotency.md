...
## 👁️ Context & Symptom: The Ghost Payment
...
* **Derivation**: The key must be deterministic and scoped to the reservation contract. For the initial charge, `IdempotencyKey = Hash(OrderID + ReservationToken + PaymentAttempt + Amount)`. For a full refund, `RefundKey = Hash(PaymentIntentID + "FULL_REFUND")`. For partial refunds, the key must include amount, currency, and refund reason to avoid collapsing distinct refund operations: `RefundKey = Hash(PaymentIntentID + "REFUND" + Amount + Currency + Reason)`.
...
CREATE TABLE payment_idempotency_log (
    idempotency_key TEXT        PRIMARY KEY,        -- Hash(OrderID+ReservationToken+Attempt+Amount) or Hash(PaymentIntentID+"FULL_REFUND")
...
1. **The Webhook Receiver**: A lightweight, highly available API endpoint whose sole job is to verify the cryptographic signature of the webhook, and save the raw JSON payload to a durable append-only log (e.g., PostgreSQL `webhook_events` table or a Kafka topic). The receiver returns `HTTP 200 OK` **only after** the webhook signature is verified and the raw event is durably persisted. If persistence fails, it should return a retryable `5xx` error so the PSP can redeliver the event.
...
3. **Phase 3 (The Collision Check)**: The Async Worker processes the event. It attempts a conditional update on the Order table:
   ...
   ```
4. **Phase 4 (The Compensation)**: The database returns `RowsAffected = 0`. The worker checks the current status:
   * If `CANCELLED_BY_SWEEPER` (or `reservation_expires_at < NOW()`) -> Initiate refund.
   * If already `PAID` -> Duplicate webhook, mark processed OK.
   * If `REFUND_PENDING` -> Idempotent no-op.
   In this scenario, the order is cancelled, so the worker detects the divergence.
...
sequenceDiagram
    ...
    Worker->>DB: 5. Attempt: Conditional PAID Update
    DB-->>Worker: 6. RowsAffected = 0 (Not Payable)
...
* **PSP Webhook Assumption**: Under normal load, most payment webhooks arrive within seconds; under degraded PSP conditions, delivery may lag by minutes.
* **Reservation Grace Period**: 10m User UI timeout + 30s technical grace period before the Sweeper cancels.
* **Webhook Ingestion Target**: The receiver should acknowledge after durable persistence within tens of milliseconds. At a moderate scale, a lean receiver can persist webhooks directly to SQL. At 10,000 webhook/sec scale, Kafka-style append-only ingestion is safer than writing every raw event directly into the primary SQL database.
...
* **The Stuck Worker / Zombie Lease**: The Async Worker picks up a webhook event, marks it `IN_PROCESSING`, and then pauses or crashes before completing. The event may appear stuck, so another worker can reclaim it after the lease expires. However, the original worker may later resume and continue with stale in-memory state. *Mitigation: Use an optimistic lease pattern. The worker sets `processing_until = NOW() + interval '5 minutes'` when claiming an event. A separate watchdog query (or the worker’s next poll cycle) reschedules any event where `status = 'IN_PROCESSING' AND processing_until < NOW()`, resetting it to `UNPROCESSED`. However, final state transitions should still use conditional updates with a lease owner or fencing token, so a stale worker cannot finalize after losing its lease. All financial side effects must be protected by durable intent rows plus deterministic idempotency keys. Polling interval: every 2–5 seconds under normal load; back off to 30 seconds during DB pressure.*
...
* **The "Oversell" vs "Refund" Dilemma**: Some businesses prefer to never cancel a paid order, even if the stock is gone, and instead put the user on a backorder list. In this design, we choose the strict-correctness path: if stock was legitimately reassigned after reservation expiry, the late payment must be refunded or moved into an explicit backorder workflow. What is not allowed is silently marking the order as fulfilled without inventory.
...
* **🏛️ Invariant**: Never lock an external system and internal database together. A webhook is not truth by itself; it is an external fact that must pass through the internal state machine. Ingest durably, process asynchronously, and compensate automatically.
