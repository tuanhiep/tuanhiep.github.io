...
In [Part 1]({{< ref "1.global_flash_sale_engine_thundering_herd.md" >}}), we built the perimeter defense. We aggressively dropped 99% of the 1,000,000 initial requests at the API Gateway using the Token Bucket algorithm. But what do we do with the admitted slice — say, 10,000 users out of the initial 1,000,000 — that we *want* to let in?

A common fallacy among engineers is the belief that if you optimize your database queries (latency), your system can handle infinite traffic (throughput). This is mathematically false. When tens of thousands of users hit your checkout API simultaneously, they will exhaust the database connection pool in milliseconds. CPU time shifts from useful work to connection management and context switching, and effective throughput collapses.
