...
That gap — between legitimate demand at the edge and safe processing capacity in the core — is where the **Virtual Waiting Room** lives. *It is not the place where rejected traffic goes; it is the buffer between eligible demand and safe core capacity.*
...
| **Eligible Users** | 100,000 users | Passed edge filtering and eligible to enter the waiting room. |
...
* Successful clients receive a signed or encrypted `Clearance Cookie`, which the API Gateway can verify cheaply without calling the core backend.
