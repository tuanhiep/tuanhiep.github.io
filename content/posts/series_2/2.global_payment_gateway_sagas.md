...
At the scale of Uber, Stripe, or Amazon, holding synchronous locks across network boundaries leads to cascading failures. **Across service boundaries, we usually give up synchronous global consistency in favor of availability and forward recovery.**
...
Because they share a transaction, the write is 100% atomic. An asynchronous relay process (like Debezium tailing the database binlog) then reads the `outbox` table and reliably publishes the messages to Kafka. *This closes the dual-write gap between the database and the message bus.*
...
**🧑‍💻 The Architect**:
If designed poorly, yes. But we do not use the Orchestrator's database for complex SQL joins or full-text searches. It is a highly optimized **Key-Value State Store**. We use `CorrelationID` as the Primary Key and perform atomic `UPSERT` operations. At higher scales, the saga state store can evolve from a relational table into a key-value or wide-column store (like Cassandra or Google Bigtable) optimized for append-heavy state transitions.
