...
1. **Card Networks via PSPs (Payment Service Providers):** When a user pays, the system immediately *authorizes* the transaction (putting a hold on funds). However, the actual *settlement* happens days later in a batch process.
...
## 🌀 2) Design Principle 2: The Payment State Machine
...
stateDiagram-v2
    ...
    CAPTURED --> SETTLED: External Settlement Confirmed (Reconciled)
...
SELECT 
    COALESCE(internal.reference_id, external.trace_id) AS correlation_id,
    CASE 
        WHEN external.trace_id IS NULL THEN 'INTERNAL_ONLY' 
        WHEN internal.reference_id IS NULL THEN 'EXTERNAL_ONLY' 
        WHEN internal.amount != external.net_amount + external.fee THEN 'AMOUNT_MISMATCH'
        WHEN internal.currency != external.currency THEN 'CURRENCY_MISMATCH'
        WHEN internal.settlement_date != external.settlement_date THEN 'DATE_MISMATCH'
        WHEN internal.status NOT IN ('SETTLED', 'RECONCILED') THEN 'STATUS_MISMATCH'
        ELSE 'IN_SYNC'
    END AS recon_status
FROM internal_ledger internal
FULL OUTER JOIN external_settlement external 
ON internal.reference_id = external.trace_id;
```
...
### ⛩️ a. Circuit Breakers (Protecting Synchronous Paths)
...
### ⛩️ b. The Dead Letter Queue (Isolating Asynchronous Poison)
...
> **🕵️ The Challenger**: What happens if a PSP successfully processes the payment, but their Webhook notifying our system fails to deliver?

**🧑‍💻 The Architect**:
Never trust the network. We implement a **scheduled polling job** querying pending statuses to catch immediate failures. If both the webhook and the polling fail, the nightly **Reconciliation Engine** will flag the discrepancy (`EXTERNAL_ONLY` - the external report has the record, but we do not). Our repair loop will then verify the external reference and either backfill the missing ledger entry or open a Finance Ops review case depending on strict financial policy.

> **🕵️ The Challenger**: During reconciliation, what happens if the external report shows a transaction that does not exist in our Internal Ledger (`EXTERNAL_ONLY`)? Should we auto-refund the customer?
...
🪷 *One sentence to trigger the reflex*: **"Webhooks are for speed, reconciliation is for truth; isolate the poison with DLQ, and open the circuit to survive the storm."**
