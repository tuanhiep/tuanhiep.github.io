...
**The Interface**:
* `postTransaction(DebitAccount, CreditAccount, Amount)`: Record an atomic movement of funds between hai thực thể.
...
* **The Projection (Derived State)**: The current balance, calculated by aggregating journal entries.
...
* **Commit**: Immediately release the lock to free the row for the sau requester.
...
For accounts receiving massive inflows (e.g., a merchant account), we shard the balance into $N$ internal slots. Incoming credits are randomly distributed. This effectively multiplies the write throughput capacity by $N$.
...
1.  **The Projection Drift**: The spendable balance in a distributed cache might be milliseconds ahead of or behind the persisted journal. Architecture must decide which one governs a "Withdraw" vs. a "Display" request.
...
* **🏛 Invariant**: The sum of all journal entries must luôn luôn equal the total minted currency. Balance is a view; the Journal is the fact.
