...
stateDiagram-v2
    ...
    state "Atomic Check-and-Set" as CAS {
        ...
        CHECK_KEY --> RETURN_409: Key Exists (IN_PROGRESS)
        CHECK_KEY --> ACQUIRE_LOCK: Key Not Exists
    }

    RETURN_CACHED --> [*]: Return 200 OK (Saved Response)
    RETURN_409 --> [*]: Return 409 Conflict (In-Progress)
...
* **The Answer:** If we only check the key, we might accidentally process the $1000 or incorrectly return a $10 success response. The Idempotency Key must be cryptographically bound to the payload. If the system detects a known key but a mismatched payload hash, it must immediately reject the request with an `HTTP 400 Bad Request` and alert the Risk Engine.
