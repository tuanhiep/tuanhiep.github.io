---
title: "Stock Exchange Core (2/3): The Single-Threaded Matching Engine"
date: 2026-02-20
...
Welcome back to the Stock Exchange Core series. In [Part 1](/posts/2.stock-exchange-1-order-book), we designed an $O(1)$ memory-aligned Order Book. But a static data structure is useless without an engine to drive it. Today, we build the **Heart**: The Matching Engine.
...
graph TD
    subgraph "Producer Layer (Network I/O - Multi-threaded)"
        N1[Gateway 1] -->|CAS Claim| RB
        N2[Gateway 2] -->|CAS Claim| RB
    end
    ...
    subgraph "Consumer Layer (Core Business Logic - Single Thread)"
        RB -->|Spin Read| ME[Matching Engine <br/> Core 1]
        ME -->|Mutates| OB[The Order Book <br/> (From Part 1)]
    end
...
## 💎 7. Optional Deep Dive: Mechanical Sympathy & False Sharing
...
> **Next up**: In [Part 3](https://www.google.com/search?q=/posts/4.stock-exchange-3-architecture), we will zoom out to the Macro level. How do we achieve **High Availability** and **Disaster Recovery** if our single-threaded engine crashes? We will explore Event Sourcing and the Sequencer pattern.
