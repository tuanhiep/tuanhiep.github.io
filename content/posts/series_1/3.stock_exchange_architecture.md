...
In [Part 1]({{< ref "1.stock_exchange_order_book.md" >}}), we built the **Order Book** in RAM.
In [Part 2]({{< ref "2.stock_exchange_matching_engine.md" >}}), we confined the **Matching Engine** to a single CPU core using the LMAX Disruptor.
...
### 🗺️ The Macro Architecture Diagram
...
## 🔮 5. The Architect's Crucible: Real-World Edge Cases
...
### 🗝️ The "Brick" Summary (Mental Model)
  ...
  * **🏛️ Invariant**: Determinism. `f(State_N, Sequence_N+1) -> State_N+1`. Identical inputs always produce identical state.
...
> **Next up**: In the Grand Finale, [Part 4]({{< ref "4.stock_exchange_tradeoffs.md" >}}), we will tear this entire architecture down. We will discuss the ultimate trade-offs, when NOT to build this system, and the alternative architectures used by modern Tech Giants.
